...
rm /home/dyu/.music-not-tagged/tmp.mp3

url="$1"

# Download and extract audio
if ! /home/dyu/scripts/yt-dlp.sh --extract-audio --audio-format mp3 --output '/home/dyu/.music-not-tagged/tmp.mp3' "$url"; then
...
if [ "$2" = "-na" ] || [ "$2" = "--no-auto" ]; then
  if ! /usr/bin/beet import -s /home/dyu/.music-not-tagged/tmp.mp3; then
      echo "Error importing into beets."
      exit 1
  fi
else 
  if ! { echo "1"; echo "A"; } | beet import -s /home/dyu/.music-not-tagged/tmp.mp3; then
      echo "Error importing into beets."
      exit 1
  fi
fi
...
if ! /home/dyu/env/bin/beet ls | /home/dyu/env/bin/beet remove -f; then
    echo "Error removing entries from beets."
    exit 1
fi

# Extract title and artist
title=$(ffprobe -show_format -v error /home/dyu/.music-not-tagged/tmp.mp3 | grep -m 1 'TAG:title' | cut -d= -f2)
artist=$(ffprobe -show_format -v error /home/dyu/.music-not-tagged/tmp.mp3 | grep -m 1 'TAG:artist' | cut -d= -f2)
...
t="$artist - $title"
destination="/home/dyu/Music/$t.mp3"
...
# Move the file
if ! mv /home/dyu/.music-not-tagged/tmp.mp3 "$destination"; then
